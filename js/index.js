...
function switchMeal(){
    var mealIndex = Math.floor(Math.random() * meals.length);
    var meal = meals[mealIndex];
    if (meal.name === "French Onion Soup" || meal.name === "BBQ Pulled Pork" || 
        meal.name === "Margherita Pizza" || meal.name === "Chicken Tikka Masala" ||
        meal.name === "Greek Moussaka" || meal.name === "Lasagna Bolognese" ||
        meal.name === "Mediterranean Quinoa Bowl" || meal.name === "Vegetable Curry")
        
         document.getElementById("timedOut").classList.remove("visually-hidden");
    else document.getElementById("timedOut").classList.add("visually-hidden");

    
    document.getElementById("name").innerText = meal.name;
    document.getElementById("description").innerText = meal.description;
    document.getElementById("image").innerHTML = `<img class="object-fit-cover w-100 h-100" src="${meal.imgCover}" alt="${meal.image}"></img>`;
    document.getElementById("prepTime").innerText = meal.prepTime;
    document.getElementById("cookTime").innerText = meal.cookTime;
    document.getElementById("servings").innerText = meal.servings;
    document.getElementById("difficulty").innerText = meal.difficulty;
    document.getElementById("category").innerText = meal.category;
    document.getElementById("ratingsAverage").innerText = meal.ratingsAverage;
    document.getElementById("ratingsQuantity").innerText = meal.ratingsQuantity;
    
    document.getElementById("ingredients").innerHTML = ``;
    for (var index = 0; index < meal.ingredients.length; index++) {
        
        document.getElementById("ingredients").innerHTML +=
                                `<span class="d-flex align-content-center gap-2 my-3"> 
                                        <small class="text-white fw-bold rounded-circle px-2 py-0" style="background-color: rgb(255 105 0);">${index+1}</small>
                                        <p class="text-dark-emphasis m-0">${meal.ingredients[index]}</p>
                                    </span>`
    }
    ...
    for (let index = 0; index < meal.instructions.length; index++) {
        document.getElementById("instructions").innerHTML += 
                                `<div class="d-flex align-items-center gap-2 mb-4"> 
                                        <span style="background-color: rgb(255 105 0);padding: 8px 18px;" class="text-white fs-5 fw-bold rounded-4">${index+1}</span>
                                        <p class="m-0 mx-2">${meal.instructions[index]}</p>
                                    </div>`;
    }
    ...
    document.getElementById("carbs").innerHTML = meal.nutrition.carbs;
    ...
    for (let index = 0; index < meal.tips.length; index++) {
        document.getElementById("tips").innerHTML += 
                                `<div class="d-flex align-items-center bg-warning-subtle  gap-2 mb-3 rounded-3 " style="padding: 20px 10px; border-left: 5px solid orange;">
                                        <i class="fa-solid fa-circle-check fw-bold fs-5" style="color: darkorange;"></i>
                                        <p class="m-0">${meal.tips[index]}</p>
                                    </div>`;
}
